...
contract ProxyStakingFactory is Ownable {
    ...
    constructor(address _nftContract, address _mvkaAggregator) Ownable(msg.sender){
    ...
    // 创建新的代理合约并铸造一个NFT
    function createProxyStakingContract(address vkaToken, address vaultkaContract, uint256 amount, uint256 duration, string memory uri) external {
        // 铸造NFT
        uint256 tokenId = nftContract.safeMint(msg.sender,uri);
        // 创建代理合约
